...
# https://docs.prefect.io/2.10.18/concepts/tasks/
...
@task(retries=2, retry_delay_seconds=5, name="calculate metrics postgresql")
def calculate_metrics_postgresql(curr, i):
	...
	curr.execute(
		"insert into dummy_metrics(timestamp, prediction_drift, num_drifted_columns, share_missing_values, target_prediction_correlation, fare_amount_quantile) values (%s, %s, %s, %s, %s, %s)",
		(begin + datetime.timedelta(i), prediction_drift, num_drifted_columns, share_missing_values, target_prediction_correlation, fare_amount_quantile)
	)
